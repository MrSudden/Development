...
function Memo() {
    ...
    const addTodo = () => {
        setTodos((t) => [...t, { id: crypto.randomUUID(), title: `Todo ${count + 1}` }]);
        setCount(count + 1);
    }

    const addNewTodo = () => {
        setNewTodos((t) => [...t, { id: crypto.randomUUID(), title: `Todo ${count + 1}` }]);
    }

    const buttonStyle = "text-white bg-cyan-600 w-50 my-2";
    const todoStyle = "flex flex-col place-items-center text-sm w-96 py-5 bg-slate-200";
        ...
        <>
            ...
            <div className="flex gap-3">
                <div className={todoStyle}>
                    ...
                    <button className={buttonStyle} onClick={addTodo}>Add Todo</button>
                    <div>Count: {count}</div>
                    <button className={buttonStyle} onClick={() => setCount(count + 1)}>+</button>
                </div>
                <div className={todoStyle}>
                    <div className="mb-2">React Hooks useMemo</div>
                    <Todos todos={newTodos} addTodo={addNewTodo} buttonStyle={buttonStyle} todoStyle={childTodoStyle} />
